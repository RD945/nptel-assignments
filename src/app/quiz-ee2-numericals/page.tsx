...
export default function QuizEe2Numericals() {
  ...
  if (!subject) {
    return (
      <SubjectSelector
        ...
        subtitle="Select a subject to start numericals-only quiz mode."
        onSelect={setSubject}
        showBackLink
      />
    );
  ...
  const questions = getQuestionsForSubject(subject).filter(isNumericalQuestion);
